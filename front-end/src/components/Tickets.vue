     ...
     <div class="problem">
      <div class="problempart">
       ...
       <div class="problempart2">
        <p>{{ticket.comment}}</p>
        <div class="actions" >
            <button @click="setEditing(ticket)" class="pure-button space-right">Edit</button>
            <form class="pure-form" v-if="ticket.editing" @submit.prevent="EditNote(ticket)">
              <fieldset>
               <textarea v-model="ticket.comment"></textarea>
               <br />
               <div class="bottons2">
                 <button @click="cancelEditing(ticket)" class="pure-button space-right">Done Editing</button>
               </div>

              </fieldset>
             </form>
          <button @click="deleteNote(ticket)" class="pure-button space-right">Delete</button>
        </div>
       </div>
     </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    deleteNote(ticket) {
      try {
        axios.delete("/api/tickets/" + ticket._id);
    ...
    editNote(ticket) {
      try {
        axios.put("/api/tickets/" + ticket._id, {comment: this.comment, editing: "false"});
        this.findTicket = null;
        this.getTickets();
        return true;
      } catch (error) {
